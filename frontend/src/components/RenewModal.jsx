...
export default function RenewModal({ member, allMembers = [], onClose, onConfirmRenew }) {
  ...
  // Dynamic UPI Intent String and QR Code generator
  const upiTxnRef = useMemo(() => `TXN-${Date.now().toString().slice(-6)}`, [currentMember?.id, selectedPlan]);
  ...
  const handleCopyUpi = () => {
    if (navigator.clipboard) {
      navigator.clipboard.writeText(dynamicUpiUrl);
      setCopiedUpi(true);
  ...
  const handleConfirm = () => {
    ...
    const finalNotes = [notes, utrRef ? `UTR: ${utrRef}` : ''].filter(Boolean).join(' | ');
    onConfirmRenew({
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fade-in">
      <div className="bg-zinc-950 border border-zinc-800 w-full max-w-xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-5 overflow-y-auto space-y-5 text-xs text-zinc-300">
          ...
          {/* Dynamic UPI QR Code & UTR Reconciliation */}
          {paymentMethod === 'UPI' && (
            <div className="p-4 bg-zinc-900/80 border border-red-500/30 rounded-2xl space-y-3">
              <div className="flex items-center justify-between border-b border-zinc-800 pb-2">
                <div className="flex items-center gap-2">
                  <QrCode className="w-4 h-4 text-red-400" />
                  <span className="text-xs font-bold text-white uppercase tracking-wider">Dynamic UPI QR Code</span>
                </div>
                <span className="text-[10px] font-mono px-2 py-0.5 bg-red-950/60 text-red-300 border border-red-500/30 rounded">
                  Ref: {upiTxnRef}
                </span>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4 pt-1">
                ...
                <div className="flex-1 space-y-2 text-xs w-full">
                  <div>
                    <span className="text-[10px] text-zinc-400 block">Gym UPI VPA:</span>
                    <span className="text-white font-mono font-bold">phoenixgym.vkp@oksbi</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-zinc-400 block">Exact Dynamic Fee:</span>
                    <span className="text-emerald-400 font-extrabold text-sm">₹{renewalAmount}</span>
                  </div>
                  <button
                    type="button"
                    onClick={handleCopyUpi}
                    className="w-full py-1.5 px-3 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 hover:text-white rounded-lg text-[10px] font-bold flex items-center justify-center gap-1.5 transition-all cursor-pointer border border-zinc-700"
                  >
                    {copiedUpi ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                    <span>{copiedUpi ? 'Copied UPI Intent URL!' : 'Copy UPI Link for Customer'}</span>
                  </button>
                </div>
              </div>
              ...
              <div className="pt-2 border-t border-zinc-800">
                <label className="block text-[10px] font-bold text-amber-400 uppercase tracking-wider mb-1">
                  Reconciliation UTR / Transaction No. (From GPay / PhonePe)
                </label>
                <input
                  type="text"
                  value={utrRef}
                  ...
                  className="w-full px-3 py-2 bg-zinc-950 border border-zinc-700 rounded-xl text-xs text-white placeholder:text-zinc-600 focus:outline-none focus:border-red-500 font-mono"
                />
              </div>
            </div>
          )}
